...
#else
    void Log(QtMsgType type, const QMessageLogContext &, const QString &msgp)
#endif
{
    ...
    QString content;
    QString msg = QString("Time: %1, %2").arg(QDateTime::currentDateTime().toString("yyyy-MM-dd HH:mm:ss.zzz")).arg(msgp);
    ...
    switch (type)
    {
        ...
        case QtWarningMsg:
            content = QString("[Warn] %1").arg(msg);
            break;
    ...
    emit SaveLog::Instance()->LogSignal(type, msg);

    if (SaveLog::Instance()->m_bSave)
    {
